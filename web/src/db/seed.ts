...
async function main() {
  ...
  await db.delete(tlsPlaybookPurchases);
  await db.delete(tlsPlaybooks);
  await db.delete(tlsCommerceJobs);
  await db.delete(tlsCommerceServices);
  await db.delete(tlsRevenues);
  await db.delete(tlsApprovals);
  await db.delete(tlsActivities);
  await db.delete(tlsPatrons);
  await db.delete(tlsTalos);
  ...
  const talosData = [
    // ── SALES Category ──
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004812753",
      tokenSymbol: "SGNL",
      pulsePrice: "2.50",
      totalSupply: 500000,
      creatorShare: 0,
    ...
    },
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004813001",
      tokenSymbol: "ENRCH",
      pulsePrice: "0.85",
      totalSupply: 2000000,
      creatorShare: 0,
    ...
    },
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004811200",
      tokenSymbol: "LEADS",
      pulsePrice: "5.00",
      totalSupply: 300000,
      creatorShare: 0,
    ...
    // ── ANALYTICS Category ──
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004814500",
      tokenSymbol: "SCOUT",
      pulsePrice: "1.20",
      totalSupply: 1000000,
      creatorShare: 0,
    ...
    },
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004810050",
      tokenSymbol: "TRND",
      pulsePrice: "0.35",
      totalSupply: 5000000,
      creatorShare: 0,
    ...
    },
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004815800",
      tokenSymbol: "AUDNC",
      pulsePrice: "8.00",
      totalSupply: 150000,
      creatorShare: 0,
    ...
    // ── DEVELOPMENT Category ──
    {
      ...
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004816200",
      tokenSymbol: "NEXUS",
      pulsePrice: "3.00",
      totalSupply: 800000,
      creatorShare: 0,
    ...
    // ── MARKETING Category ──
    {
      ...
      category: "Marketing",
      description:
        "AI agent that reviews products and shares honest, structured reviews across developer communities. Powered by OpenClaw — the first Talos agent running on an external agent framework.",
      status: "Active",
      pulseTokenAddress: "0x0000000000000000000000000000000004817300",
      tokenSymbol: "VOICE",
      pulsePrice: "1.80",
      totalSupply: 600000,
      creatorShare: 0,
  ...
  for (const c of talosData) {
    const [talos] = await db
      ...
      .values({
        ...
        status: c.status,
        pulseTokenAddress: (c as any).pulseTokenAddress ?? null,
    ...
    // Patrons — each talos gets a unique mix of patron types
    ...
    console.log(`Seeded: ${talos.name} (${talos.id}) — ${patrons.length} patrons, Pulse $${c.pulsePrice}`);
  }

  // ─── Playbook Data ──────────────────────────────────────
  const allTaloses = await db
    .select({ id: tlsTalos.id, name: tlsTalos.name })
    .from(tlsTalos);
  const talosMap = new Map(allTaloses.map((t) => [t.name, t.id]));

  const playbookData = [
    {
      talosName: "Prospect Signal",
  ...
  for (const pb of playbookData) {
    const talosId = talosMap.get(pb.talosName);
    if (!talosId) continue;

    await db.insert(tlsPlaybooks).values({
      talosId,
      title: pb.title,

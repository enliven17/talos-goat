...
export const tlsTalos = pgTable(
  "tls_talos",
  {
    ...
    onChainId: integer("onChainId").unique(),        // GOAT TalosRegistry TALOS ID
    ...
    // Pulse Token (GOAT ERC-20 PulseToken)
    pulseTokenAddress: text("pulseTokenAddress"),     // ERC-20 contract address (0x...) for the Pulse token
    ...
    // Agent EVM Wallet (keypair — private key stored server-side, never in DB)
    agentWalletId: text("agentWalletId"),             // EVM address (0x...) — wallet identifier
    agentWalletAddress: text("agentWalletAddress"),   // EVM address (0x...) — for display/payment routing
...
export const tlsPatrons = pgTable(
  "tls_patrons",
  {
    ...
    walletAddress: text("walletAddress").notNull(),   // EVM address (0x...)
  ...
  (t) => [
    uniqueIndex("tls_patrons_talosId_walletAddress_key").on(t.talosId, t.walletAddress),
  ],
);
...
export const tlsCommerceServices = pgTable(
  "tls_commerce_services",
  {
    ...
    currency: text("currency").notNull().default("USDC"),
    walletAddress: text("walletAddress").notNull(),   // Payment recipient (EVM 0x... address)
    chains: text("chains").array().notNull().default(["goat"]),
...
// ─── Commerce Job (x402 Job Queue — GOAT) ─────────────────────────
...
export const tlsCommerceJobs = pgTable(
  "tls_commerce_jobs",
  {
    ...
    result: jsonb("result"),
    status: text("status").notNull().default("pending"),
    paymentSig: text("paymentSig").unique(),   // GOAT x402 payment token hash (replay prevention)
    txHash: text("txHash"),                    // GOAT transaction hash after settlement
    amount: numeric("amount", { precision: 18, scale: 6 }).notNull(),
...
export const tlsPlaybookPurchases = pgTable(
  "tls_playbook_purchases",
  {
    ...
    buyerAddress: text("buyerAddress").notNull(),   // EVM 0x... address

...
import { createAgentKeypair, fundTestnetAccount } from "@/lib/goat";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create EVM keypair for the agent wallet (GOAT Network).
    // Keypair creation happens BEFORE the DB transaction.
    // If the transaction later fails, the keypair is simply discarded — an
    // unfunded EVM address has no on-chain state, so this is safe.
    // The private key is generated here but NEVER stored in the DB; the operator
    // persists it server-side (env var TALOS_AGENT_SECRET_<id> or secret manager).
    ...
    try {
      ...
      agentWalletPublicKey = keypair.address;
      // keypair.privateKey is intentionally not persisted to the DB.
    } catch (err) {
    ...
    const { talos, generatedKey } = await db.transaction(async (tx) => {
      const [talos] = await tx
        ...
        .values({
          ...
          pulseTokenAddress: pulseTokenAddress ?? null,
    ...
    // Kept outside the transaction deliberately: the GOAT faucet is an external
    // call and must not cause a DB rollback if it fails.

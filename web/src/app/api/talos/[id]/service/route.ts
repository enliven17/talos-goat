...
import { verifyX402Payment, settleX402Payment } from "@/lib/x402";
import { fulfillInstant } from "@/lib/fulfillment";
import { registerServiceSchema, parseBody } from "@/lib/schemas";

const GOAT_NETWORK = process.env.GOAT_NETWORK ?? "testnet";
...
export async function GET(
  ...
  try {
    ...
    // payee: use service walletAddress if set, otherwise fall back to agent wallet
    ...
    // Return 402 Payment Required with x402 GOAT payment details
...
export async function POST(
  ...
  try {
    ...
    // 2. Validate X-PAYMENT header (GOAT x402 token)
    const paymentHeader = request.headers.get("x-payment");
    if (!paymentHeader) {
      return Response.json(
        { error: "Missing X-PAYMENT header with GOAT x402 payment token" },
        { status: 400 }
      );
    }
    ...
    // 5. Settle x402 payment on-chain (submits the tx via the GOAT facilitator)
    let txHash: string;
    try {
      const result = await settleX402Payment(paymentToken);
      txHash = result.txHash;
    } catch (settleErr) {
      console.error("GOAT x402 settlement failed:", settleErr);
      return Response.json(

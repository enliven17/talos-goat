...
// POST /api/talos/:id/sign — Signing proxy for GOAT x402 payments
// Agent sends payment details, Web signs server-side, returns a payment token
export async function POST(
  ...
  try {
    ...
    const { payee, amount, token } = parsed.data;
    ...
    // 6. Sign x402 payment via GOAT (USDC ERC-20 by default).
    // `token` is an optional ERC-20 address; omit to use the configured USDC.

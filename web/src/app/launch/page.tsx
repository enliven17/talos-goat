...
import {
  ...
  TALOS_REGISTRY_ADDRESS,
  TALOS_NAME_SERVICE_ADDRESS,
  REGISTRY_ABI,
  NAME_SERVICE_ABI,
} from "@/lib/contracts";

// Treasury / operator EVM address that receives the treasury share on-chain.
...
export default function LaunchPage() {
  return (
    <WalletGate
      ...
      description="Creating a TALOS requires a GOAT Network wallet connection. Your address will be registered as the Creator."
    >
      <LaunchForm />
    </WalletGate>
  );
...
function LaunchForm() {
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      setDeployStep("Verifying GOAT wallet...");
      setDeployProgress(1);
      if (!publicClient) throw new Error("RPC client unavailable");

      const creatorAddr = getAddress(form.creatorWallet || address);
      const treasuryAddr = getAddress(OPERATOR_ADDRESS);
      ...
      if (TALOS_REGISTRY_ADDRESS) {
        setDeployStep("Registering TALOS on GOAT...");
        setDeployProgress(2);

        const registryAddress = getAddress(TALOS_REGISTRY_ADDRESS);

        // The new TALOS id is the registry's nextTalosId at submit time.
        let predictedId: bigint;
        try {
          predictedId = (await publicClient.readContract({
          ...
          predictedId = BigInt(0);
        }

        // Struct tuples — keep existing numeric conventions:
        //   shares as integer percentages, price as USD cents, budgets/thresholds as whole units.
        const patron = {
          creatorShare: 60,
          investorShare: 25,
          treasuryShare: 15,
        ...
        const pulse = {
          totalSupply: BigInt(Math.round(Number(form.totalSupply) || 1_000_000)),
          priceUsdCents: BigInt(Math.round(parseFloat(form.initialPrice || "0.01") * 100)),
        ...
        if (receipt.status !== "success") throw new Error("createTalos transaction reverted");

        onChainId = predictedId > BigInt(0) ? Number(predictedId) : null;
      } else {
      ...
      if (TALOS_NAME_SERVICE_ADDRESS && onChainId !== null) {
      ...
      if (raw.includes("User declined") || raw.includes("user rejected"))
        ...
        message = "Insufficient BTC balance for gas on this transaction.";
      ...
      <div className="bg-surface border border-border p-8 mb-6">
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              The Creator is your GOAT Network address. It is permanently linked to this TALOS and cannot be changed.
            </p>
            <div>
              <label className="block text-xs text-muted mb-2">Creator Address (GOAT)</label>
              ...
              <p className="text-xs text-muted mt-1">This GOAT Network address will be registered as the TALOS Creator on-chain.</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              Configure your TALOS&apos;s Mitos token on GOAT Network.
            </p>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">2-8 characters, letters and numbers only. Used as the token symbol.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">Price per Pulse token in USDC on GOAT Network</p>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              <ReviewRow label="Creator (GOAT)" value={form.creatorWallet || address || ""} />
